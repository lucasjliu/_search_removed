...
void SearchImpl::doSearch(std::vector<std::string>&& query,
              int k,
              std::vector<result_t>& res)
{
    QueryProcessFunc doc_at_a_time = &Alg::doc_a_time;
    ScoreAlg bm25 = {Alg::bm25_term, Alg::score_aggr_add};
    ...
    {
        res.emplace_back(std::move(_genResult(doc)));
    }
}
...
score_t SearchImpl::Alg::bm25_term(const word_t& term,
                                   doc_id_t docid,
                                   doc_len_t freq,
                                   const InvertedIndex& index,
                                   const DocCollection& docs)
{
    doc_len_t dl = docs.select(docid).len;
    score_t k1 = 1.2; //2.0
    score_t b = 0.75;
    score_t wi = log(((double)docs.num - (double)freq + 0.5) / ((double)freq + 0.5));
    score_t ri =  freq * (k1 + 1) / (freq + k1*(1 - b + b * dl/docs.avgdl));
    return wi * ri;
}
    ...
    while (doc < N)
    {
        ...
        for (size_t i = 0; i < query.size(); ++i)
        {
            ...
            if (currDocId == doc)
            {
                ...
                score_alg.score_aggr_func(temp.score, term_score);
            }*/
            if (index.find(query[i]) != index.end())
            {
                if (index.find(query[i])->second.hasHit(doc))
                {
                    doc_len_t freq = index.find(query[i])->second.getHit(doc).freq;
                    score_t term_score = score_alg.term_score_func(query[i], doc, freq, index, docs);
                    score_alg.score_aggr_func(temp.score, term_score);
                }
            }
